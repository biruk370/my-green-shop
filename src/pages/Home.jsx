    ...
    return (
        <div style={{ padding: '20px',background:'#B0FFFA' }}>
           ...
           <button style={styles.primaryBtn} onClick={() => document.getElementById('products-section').scrollIntoView({ behavior: 'smooth' })}>Shop Now<ArrowRight size={20} /></button> 
            ...
            <h1 style={{ textAlign: 'center', marginBottom: '30px', color: '#333' }}>Selected Product</h1>
            <div style={styles.productGrid}>
                {
                    allProducts && allProducts.map((product) => (
                        <div key={product.id} style={styles.productCard}>
                            ...
                            <p style={{ color: '#27ae60', fontWeight: 'bold',fontSize: '1.2rem' }}>{product.price}ETB</p>
                            <button style={styles.addToCartBtn} onClick={() => addToCart(product)}>Add to Cart</button>
                        </div>
                    ))
                }
            </div> 
